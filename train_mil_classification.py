...
def _raise_on_projection_mismatch(
    missing_keys,
    checkpoint_path: Path,
    embedding_dim: int,
) -> None:
    projection_missing = sorted(k for k in missing_keys if str(k).startswith("projection."))
    if not projection_missing:
        return
    raise ValueError(
        "Encoder checkpoint '{}' did not provide {}. "
        "This usually means --embedding-dim does not match the pretrained encoder checkpoint. "
        "Use the native checkpoint embedding dimension, typically 1024 for ECGFounder-derived encoders, instead of {}.".format(
            checkpoint_path,
            projection_missing,
            embedding_dim,
        )
...
def _build_model(args, device: torch.device) -> ECGModel_Attn:
    ...
    if args.encoder_ckpt:
        ...
        _raise_on_projection_mismatch(missing, args.encoder_ckpt, args.embedding_dim)
        print(
            json.dumps(
                {
...
def build_argparser():
    ...
    parser.add_argument("--embedding-dim", type=int, default=1024, help="Encoder embedding dimension. Keep this aligned with the pretrained encoder checkpoint; ECGFounder-derived encoders are typically 1024.")
