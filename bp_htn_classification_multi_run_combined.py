...
Default splits/thresholds:
    ...
    combined: subject-level OR rule; positive if all OR day OR night is positive
...
def _label_subject_from_valid_rows(
    subject_rows: List[ValidRow],
    threshold: Threshold,
) -> Optional[LabeledItem]:
    """Create one subject-level label from a list of valid rows.

    Returns None if the subject has no valid rows for this split.
    """
    if not subject_rows:
        return None
...
def compute_combined_subject_metrics(
    rows: Iterable[Dict[str, str]],
    ...
    *,
    ...
    """Compute the combined hypertension label per subject.

    Rule:
        true_combined_positive = true_all_positive OR true_day_positive OR true_night_positive
        pred_combined_positive = pred_all_positive OR pred_day_positive OR pred_night_positive

    Important:
        This is a subject-level product rule. It is not a new BP threshold.
        If a subject has no day rows or no night rows, that missing split is ignored.
    """
    rows_list = list(rows)

    valid_all = collect_valid_rows(
        rows_list,
        lambda row: True,
        subject_col,
        pred_sbp_col,
    ...
    valid_night = collect_valid_rows(
        rows_list,
    ...
    grouped_all: DefaultDict[str, List[ValidRow]] = defaultdict(list)
    grouped_day: DefaultDict[str, List[ValidRow]] = defaultdict(list)
    grouped_night: DefaultDict[str, List[ValidRow]] = defaultdict(list)

    for row in valid_all:
        grouped_all[row.subject_id].append(row)
    for row in valid_day:
        grouped_day[row.subject_id].append(row)
    for row in valid_night:
        grouped_night[row.subject_id].append(row)

    subject_ids = sorted(set(grouped_all) | set(grouped_day) | set(grouped_night))

    labeled_subjects: List[LabeledItem] = []
    for sid in subject_ids:
        split_items: List[LabeledItem] = []

        item_all = _label_subject_from_valid_rows(grouped_all.get(sid, []), all_threshold)
        item_day = _label_subject_from_valid_rows(grouped_day.get(sid, []), day_threshold)
        item_night = _label_subject_from_valid_rows(grouped_night.get(sid, []), night_threshold)

        for item in [item_all, item_day, item_night]:
            if item is not None:
        ...
        labeled_subjects.append(
            LabeledItem(
                true_positive=any(item.true_positive for item in split_items),
                pred_positive=any(item.pred_positive for item in split_items),
            )
        )

    counts = accumulate_counts(labeled_subjects)

    # threshold_sbp/dbp are NaN because combined is an OR of three threshold systems.
    return metrics_from_counts(
        run_id=run_id,
        csv_path=csv_path,
        method=method,
        split="combined",
        mode="subject",
        threshold=Threshold(float("nan"), float("nan")),
        n_rows=len(valid_all),
        n_subjects=len(labeled_subjects),
        counts=counts,
    )
...
def compute_metrics_for_csv(
    ...
    for method in methods:
        ...
        if include_combined:
            out.append(
                compute_combined_subject_metrics(
                    rows=rows,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable the subject-level combined rule. By default, the script also reports "
            "combined hypertension: positive if all OR day OR night is positive."
        ),
    )
...
def main() -> None:
    ...
    print(f"Combined split included: {'no' if args.no_combined else 'yes'}")
