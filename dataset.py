...
try:
    ...
    from . import ecg_preprocess as ep
...
def _robust_rescale_1d(signal: np.ndarray, clip_value: float = 8.0) -> np.ndarray:
    ...
    scale = float(np.quantile(abs_centered, 0.95))
    if scale <= 1e-6:
        scale = float(np.std(centered))
    if scale <= 1e-6:
        scale = float(np.max(abs_centered))
    if scale <= 1e-6:
        scale = 1.0

    centered /= scale
    np.clip(centered, -clip_value, clip_value, out=centered)
...
def preprocess_ecg(signal: np.ndarray, fs: int) -> np.ndarray:
    ...
    if signal.ndim == 1:
        scaled = _robust_rescale_1d(signal)
        cleaned = ep.pp(scaled)
    ...
    if signal.ndim == 2:
        ...
        for idx in range(lead_num):
            cleaned[:, idx] = ep.pp(_robust_rescale_1d(samples_first[:, idx]))
