...
def fit_affine_ridge(
    ...
    """
    Legacy affine ridge used in the previous working script.
    ...
    By default only the slope is penalized and the intercept is unpenalized.
    This intentionally matches the user's previous affine calibration code.
    """
    x = np.asarray(x, dtype=np.float64)
    ...
    R = np.diag([float(lam), float(lam) if penalize_intercept else 0.0])

    try:
        w = np.linalg.solve(X.T @ X + R, X.T @ y)
        a, b = float(w[0]), float(w[1])
    except np.linalg.LinAlgError:
        # Safe fallback: bias-only.
        a, b = 1.0, float(np.mean(y - x))

    return a, b


def fit_affine_identity_prior(
    x: Sequence[float],
    y: Sequence[float],
    lam: float = 10.0,
    slope_bounds: Tuple[float, float] = (0.65, 1.80),
) -> Tuple[float, float]:
    """
    Optional safer identity-prior affine.

    This is NOT the default, because the default should reproduce the previous
    working affine script. Use this only if you intentionally want slope shrinkage
    toward 1.0 with centered variables.
    """
    x = np.asarray(x, dtype=np.float64)
    y = np.asarray(y, dtype=np.float64)
...
def fit_or_bias_only(
    x: Sequence[float],
    y: Sequence[float],
    min_points_for_slope: int = 5,
    min_range_for_slope: float = 8.0,
    lam: float = 10.0,
) -> Tuple[float, float]:
    """
    Optional conservative affine: use bias-only when calibration points are too
    few or raw prediction range is too narrow.
    """
    x = np.asarray(x, dtype=np.float64)
    y = np.asarray(y, dtype=np.float64)
    ...
    if len(x) == 0:
        return 1.0, 0.0
    if len(x) < min_points_for_slope:
        return 1.0, float(np.mean(y - x))
    if (np.max(x) - np.min(x)) < min_range_for_slope:
        return 1.0, float(np.mean(y - x))

    return fit_affine_identity_prior(x, y, lam=lam)
...
def apply_affine_calibration(
    ...
    min_points: int = 3,
    lam: float = 10.0,
    slope_clip: Optional[Tuple[float, float]] = None,
    penalize_intercept: bool = False,
    fit_mode: str = "legacy_ridge",
) -> pd.DataFrame:
    """
    Affine calibration applied per subject, optionally per (subject, sleep).

    Default fit_mode='legacy_ridge' exactly matches the previous working script:
        fit_affine_ridge(x, y, lam=lam, penalize_intercept=penalize_intercept)

    Adds:
        y_pred_sbp_aff, y_pred_dbp_aff
        aff_sbp_a, aff_sbp_b, aff_dbp_a, aff_dbp_b

    Notes:
        - Evaluation should be done on is_calib == False.
        - This function uses y_pred_*_raw columns in the lazy pipeline.
    """
    out = df.copy()
    ...
    def _fit(x, y):
        if fit_mode == "legacy_ridge":
            return fit_affine_ridge(x, y, lam=lam, penalize_intercept=penalize_intercept)
        if fit_mode == "identity_prior":
            return fit_affine_identity_prior(x, y, lam=lam)
        if fit_mode == "safe_identity":
            return fit_or_bias_only(x, y, lam=lam)
        raise ValueError(f"Unknown affine fit_mode={fit_mode}")
    ...
    for key, g_all in out.groupby(group_cols, sort=False):
        ...
        a_s, b_s = _fit(g_cal["y_pred_sbp_raw"], g_cal["y_true_sbp"])
        a_d, b_d = _fit(g_cal["y_pred_dbp_raw"], g_cal["y_true_dbp"])
        ...
        idx = g_all.index
        out.loc[idx, "y_pred_sbp_aff"] = (
            a_s * out.loc[idx, "y_pred_sbp_raw"].astype(float) + b_s
        )
        out.loc[idx, "y_pred_dbp_aff"] = (
            a_d * out.loc[idx, "y_pred_dbp_raw"].astype(float) + b_d
        )
...
def apply_all_calibrations(df: pd.DataFrame, emb: Optional[np.ndarray] = None, run_bias: bool = True, run_affine: bool = True, run_bank: bool = True, by_sleep_calibration: bool = False, affine_lam: float = 100.0, affine_min_points: int = 3, bank_temperature: float = 0.30, bank_time_weight_per_hour: float = 0.20, bank_sleep_bonus: float = 0.25, bank_uniform_mix: float = 0.20, bank_residual_clip: float = 35.0) -> pd.DataFrame:
    ...
    if run_affine:
        out = apply_affine_calibration(out, by_sleep=by_sleep_calibration, min_points=affine_min_points, lam=affine_lam, slope_clip=None, fit_mode='legacy_ridge')
    if run_bank:
