...
def fit_affine_identity_prior(x, y, lam=10.0, slope_bounds=(0.7, 1.3)):
    """
    Fit y ≈ a*x + b, but regularize a toward 1.0, not 0.0.
    Uses centered variables to avoid slope/intercept coupling.
    """
    x = np.asarray(x, dtype=np.float64)
    y = np.asarray(y, dtype=np.float64)
    ...
    # yc ≈ a * xc, with prior a ~ 1
    # let d = a - 1  =>  yc - xc ≈ d * xc
    num = np.sum(xc * (yc - xc))
    den = np.sum(xc * xc) + lam
...
def fit_or_bias_only(x, y, min_points_for_slope=5, min_range_for_slope=8.0, lam=10.0):
    x = np.asarray(x, dtype=np.float64)
    y = np.asarray(y, dtype=np.float64)
